...
for CORES in $(seq 2 48); do
    ...
    for ALG in "${ALGORITHMS[@]}"; do
        ...
        OUTPUT=$(mpirun --mca coll_tuned_use_dynamic_rules true --mca coll_tuned_bcast_algorithm $ALG -np $CORES --map-by socket $OSU_BCAST_PATH -x $WARMUP_ITER -i $TOTAL_ITER -m $MESSAGE_SIZE:$MESSAGE_SIZE)
